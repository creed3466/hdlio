#!/bin/bash
# run_avia_debug_dark02.sh — Dark02 determinism + ATE smoke (Iter 2i C7)
#
# Runs Dark02 at -r 1.0 with TOFSLAM_DEBUG_DETERMINISM=1, 3 runs across
# 3 containers in parallel. Used as A1 (on-path determinism) and A2/A3
# (ε-bound + positive-signal) gates for the C7 PVMap-only keyframe gate
# (Iter 2i, Task #71). Dark02 first activation per architect §2 (slow
# regime, τ_t = 0.10 m).
#
# Distribution: 3-run gate — p1=1, p2=2, p3=3 (single run per container)
#
# Usage: bash docker/run_avia_debug_dark02.sh [LABEL]

set -e
cd "$(dirname "$0")/.."

LABEL="${1:-avia_debug_dark02_$(date +%Y%m%d_%H%M)}"
OUT_BASE="dump/${LABEL}"
HOST_SURFEL="/home/euntae/Project/dataset/ros1/surfel_data"

SEQ="Dark02"
CFG="avia_v6_seq/dark02.yaml"
RATE="1.0"
