#!/bin/bash
# run_avia_debug_vi03.sh — Varying-illu03 determinism validation (iter2g-a Step 7 G3)
#
# Runs 3x Varying-illu03 at -r 1.0 with TOFSLAM_DEBUG_DETERMINISM=1.
# Used for G3 no-regression evaluation of frontend_enable_igicp flag.
#
# RATE=1.0 is mandatory per CLAUDE.md determinism SOP:
#   - Varying-illu03 is a 17-minute bag; RATE > 1.0 causes CV > 0% (wall-clock
#     buffer exhaustion observed in Varying-illu03 5x runs).
...
#
# Usage: bash docker/run_avia_debug_vi03.sh [LABEL]

set -e
cd "$(dirname "$0")/.."

LABEL="${1:-avia_debug_vi03_$(date +%Y%m%d_%H%M)}"
OUT_BASE="dump/${LABEL}"
HOST_SURFEL="/home/euntae/Project/dataset/ros1/surfel_data"

SEQ="Varying-illu03"
CFG="avia_v6_seq/varying_illu03.yaml"
RATE="1.0"
