#!/bin/bash
# run_R8_F6_canonical.sh — T5.4-R8 F.6 canonical eval (Avia outdoor 9-seq, rate=1.0)
# Test: 9-seq mean ATE ≤ 0.314m (Faster-LIO baseline) + per-seq ≤ V6a×1.10.
# Uses R-A joint predicate (degen<=1 AND rho_1>=0.50) → CLASS_D.
set -e
cd "$(dirname "$0")/.."

LABEL="${1:-R8_F6_$(date +%Y%m%d_%H%M)}"
OUT_ROOT="dump/${LABEL}"
...
TIMEOUT_S=1800  # 30min per seq @ rate=1.0

CONTAINERS=(tofslam_R8F6_1 tofslam_R8F6_2 tofslam_R8F6_3)
...
log "==== T5.4-R8 F.6 CANONICAL ($(date)) ===="
log "9 outdoor seqs | rate=${RATE} | config=${CONFIG}"
log "Target: 9-seq mean ≤ 0.314m, per-seq ≤ V6a×1.10"
...
TOTAL_ATE=0
PASS_COUNT=0
log "seq, ATE, V6a×1.10, status, Stage_lock"
for seq in "${ALL_SEQS[@]}"; do
  ...
  log "  $seq: ATE=$ate, gate=$gate ($status), $lock"
  if [[ "$ate" != "NaN" ]]; then
...
log "Per-seq pass: $PASS_COUNT/9 (target 9/9)"
