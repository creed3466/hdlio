#!/bin/bash
# run_avia_debug_occ04.sh — Occlusion04 determinism + ATE (Iter 2p re-baseline)
#
# Runs 3x Occlusion04 at -r 1.0 with TOFSLAM_DEBUG_DETERMINISM=1.
# RATE=1.0 is mandatory per CLAUDE.md determinism SOP.
#
# Distribution: 3-run gate — p1=1, p2=2, p3=3 (single run per container)
#
# Usage: bash docker/run_avia_debug_occ04.sh [LABEL]

set -e
cd "$(dirname "$0")/.."

LABEL="${1:-avia_debug_occ04_$(date +%Y%m%d_%H%M)}"
OUT_BASE="dump/${LABEL}"
HOST_SURFEL="/home/euntae/Project/dataset/ros1/surfel_data"

SEQ="Occlusion04"
CFG="avia_v6_seq/occlusion04.yaml"
RATE="1.0"
N_RUNS=3
