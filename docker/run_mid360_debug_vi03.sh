#!/bin/bash
# run_mid360_debug_vi03.sh — Mid360 Varying-illu03 determinism validation
#
# Runs Varying-illu03 at -r 1.0 with TOFSLAM_DEBUG_DETERMINISM=1, 3 runs across
# 3 containers in parallel. Mid360 variant of run_avia_debug_vi03.sh.
#
# RATE=1.0 is mandatory per CLAUDE.md determinism SOP:
#   - Varying-illu03 is a 17-minute bag; RATE > 1.0 causes CV > 0%.
#   - All conclusions from this script are valid only at RATE=1.0.
#
# Container names: tofslam_m360_db_{1,2,3} (avoid collision with Avia)
# Config: ros1_m3dgr_mid360.yaml (common config, not per-seq)
#
# Distribution: 3-run gate — p1=1, p2=2, p3=3 (single run per container)
#
# Usage: bash docker/run_mid360_debug_vi03.sh [LABEL]

set -e
cd "$(dirname "$0")/.."

LABEL="${1:-m360_debug_vi03_$(date +%Y%m%d_%H%M)}"
OUT_BASE="dump/${LABEL}"
...
RATE="1.0"
N_RUNS=3  # 3-run CV gate per docs/requirements.md

echo "======== Mid360 Debug Multi-Run: ${N_RUNS}x ${SEQ} @ -r ${RATE}: $(date) ========"
echo "Output: ${OUT_BASE}/"
mkdir -p "${OUT_BASE}"

# Cleanup
for c in tofslam_m360_db_1 tofslam_m360_db_2 tofslam_m360_db_3; do
...
for i in 1 2 3; do
  cpus="$(( (i-1)*4 ))-$(( i*4-1 ))"
  docker run -d --rm --name "tofslam_m360_db_$i" \
...
build_one tofslam_m360_db_1 &
BP1=$!
build_one tofslam_m360_db_2 &
BP2=$!
build_one tofslam_m360_db_3 &
BP3=$!
wait $BP1 $BP2 $BP3
echo "[Build] Done."
...
run_list tofslam_m360_db_1 11311 "${RUNS_P1[@]}" &
PID1=$!
run_list tofslam_m360_db_2 11312 "${RUNS_P2[@]}" &
PID2=$!
run_list tofslam_m360_db_3 11313 "${RUNS_P3[@]}" &
PID3=$!
wait $PID1 $PID2 $PID3
