#!/bin/bash
# run_avia_debug_dyn04.sh — Dynamic04 determinism + ATE smoke (Iter 2m D04-L1)
#
# Runs Dynamic04 at -r 1.0 with TOFSLAM_DEBUG_DETERMINISM=1, 3 runs across
# 3 containers in parallel. Used as A1 (on-path determinism) and A2/A3
# (positive-signal) gates for the D04-L1 max_plane_distance sweep
# (Iter 2m, Task #71). RATE pinned to 1.0 (deterministic regime per
# CLAUDE.md "Rate 정책"); all sweep probes must produce CV = 0%.
#
# Spec: docs/reports/task71_iter2m_architect.md §7.3
# Anchor: 5fd7efc (Iter 2l close)
#
# Distribution: 3-run gate — p1=1, p2=2, p3=3 (single run per container)
#
# Usage: bash docker/run_avia_debug_dyn04.sh [LABEL]

set -e
cd "$(dirname "$0")/.."

LABEL="${1:-avia_debug_dyn04_$(date +%Y%m%d_%H%M)}"
OUT_BASE="dump/${LABEL}"
HOST_SURFEL="/home/euntae/Project/dataset/ros1/surfel_data"

SEQ="Dynamic04"
CFG="avia_v6_seq/dynamic04.yaml"
RATE="1.0"
...
# Run-list per container (3 runs distributed across 3 containers)
RUNS_P1=(1)
RUNS_P2=(2)
RUNS_P3=(3)
