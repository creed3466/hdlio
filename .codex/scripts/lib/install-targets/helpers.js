...
function isForeignPlatformPath(sourceRelativePath, adapterTarget) {
  ...
  for (const [prefix, ownerTarget] of Object.entries(PLATFORM_SOURCE_PATH_OWNERS)) {
    if (normalizedPath === prefix || normalizedPath.startsWith(`${prefix}/`)) {
      const sameOwner = ownerTarget === adapterTarget || (ownerTarget === 'cursor' && adapterTarget === 'cursor-cli');
      return !sameOwner;
    }
  }
...
// RCC: Directory names skipped during install enumeration.
// Centralized so all listings (rules, agents, skills, commands) honor it.
...
function shouldSkipEntry(name) {
  if (!name) return true;
  if (SKIPPED_DIR_NAMES.has(name)) return true;
  // Skip any dotfile/dotdir at install enumeration level (preserve only .claude-plugin etc.
  // which are handled at PLATFORM_SOURCE_PATH_OWNERS level — those don't appear inside
  // rules/agents/skills/commands subtrees).
  return false;
}
