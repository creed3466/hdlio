...
function getManagedBlockMarkers(operation) {
  return {
    beginMarker: operation.beginMarker || '<!-- BEGIN RCC -->',
    endMarker: operation.endMarker || '<!-- END RCC -->',
  };
}

function replaceManagedBlock(existing, managedContent, beginMarker, endMarker) {
  const start = existing.indexOf(beginMarker);
  const finish = start >= 0 ? existing.indexOf(endMarker, start) : -1;

  if (start >= 0 && finish >= 0) {
    const before = existing.slice(0, start).trimEnd();
    const after = existing.slice(finish + endMarker.length).replace(/^\s*\n?/, '').trimStart();
    return [before, managedContent.trimEnd(), after]
      .filter(part => part.length > 0)
      .join('\n\n') + '\n';
  }

  if (existing.trim().length === 0) {
    return `${managedContent.trimEnd()}\n`;
  }

  return `${existing.trimEnd()}\n\n${managedContent.trimEnd()}\n`;
}

function removeManagedBlock(existing, beginMarker, endMarker) {
  const start = existing.indexOf(beginMarker);
  const finish = start >= 0 ? existing.indexOf(endMarker, start) : -1;
  ...
  return `${existing.slice(0, start)}${existing.slice(finish + endMarker.length)}`
    .replace(/\n{3,}/g, '\n\n')
    .trim();
}
...
function shouldRepairFromRecordedOperations(state) {
  return getManagedOperations(state).some(operation => (
    !['copy-file', 'render-template', 'merge-managed-block'].includes(operation.kind)
    || (operation.kind === 'render-template' && getOperationTextContent(operation) !== null)
    || (operation.kind === 'merge-managed-block' && getOperationTextContent(operation) !== null)
  ));
}
...
function executeRepairOperation(repoRoot, operation) {
  ...
  if (operation.kind === 'merge-managed-block') {
    const managedContent = getOperationTextContent(operation);
    if (managedContent === null) {
      throw new Error(`Missing managed content for repair: ${operation.destinationPath}`);
    }
    const { beginMarker, endMarker } = getManagedBlockMarkers(operation);
    const existing = fs.existsSync(operation.destinationPath)
      ? readFileUtf8(operation.destinationPath)
      : '';

    ensureParentDir(operation.destinationPath);
    fs.writeFileSync(
      operation.destinationPath,
      replaceManagedBlock(existing, managedContent, beginMarker, endMarker)
    );
    return;
  }
...
function executeUninstallOperation(operation) {
  ...
  if (operation.kind === 'merge-managed-block') {
    ...
    const { beginMarker, endMarker } = getManagedBlockMarkers(operation);
    const existing = readFileUtf8(operation.destinationPath);
    const next = removeManagedBlock(existing, beginMarker, endMarker);
    ...
    ensureParentDir(operation.destinationPath);
    fs.writeFileSync(operation.destinationPath, `${next}\n`);
    return {
      removedPaths: [],
      cleanupTargets: [],
    };
...
function inspectManagedOperation(repoRoot, operation) {
  ...
  if (operation.kind === 'merge-managed-block') {
    ...
    const { beginMarker, endMarker } = getManagedBlockMarkers(operation);
    const expected = replaceManagedBlock('', managedContent, beginMarker, endMarker).trim();
    const actual = readFileUtf8(destinationPath);
