...
const SUPPORTED_INSTALL_TARGETS = ['claude', 'codex', 'cursor', 'cursor-cli', 'copilot'];
const INSTALL_TARGET_ALIASES = Object.freeze({
  'cursor-cli': 'cursor',
});
...
function normalizeInstallTarget(target) {
  ...
  return INSTALL_TARGET_ALIASES[normalized] || normalized;
}

function moduleSupportsTarget(module, target) {
  const moduleTargets = readModuleTargetsOrThrow(module);
  const canonicalTarget = normalizeInstallTarget(target);

  return moduleTargets.includes(target) || moduleTargets.includes(canonicalTarget);
}
...
function intersectTargets(modules) {
  ...
  return SUPPORTED_INSTALL_TARGETS.filter(target => (
    modules.every(module => Array.isArray(module.targets) && moduleSupportsTarget(module, target))
  ));
}
...
function resolveLegacyCompatibilitySelection(options = {}) {
  ...
  const baseModuleIds = LEGACY_COMPAT_BASE_MODULE_IDS_BY_TARGET[normalizeInstallTarget(target || 'claude')]
...
function resolveInstallPlan(options = {}) {
  ...
  function resolveModule(moduleId, dependencyOf, rootRequesterId) {
    ...
    const supportsTarget = !target
      || (
        moduleSupportsTarget(module, target)
