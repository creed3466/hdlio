...
function materializeScaffoldOperation(sourceRoot, operation) {
  if (operation.kind === 'render-template' || operation.kind === 'merge-managed-block') {
    const materializedOperation = {
      ...operation,
      scaffoldOnly: false,
    };

    if (typeof operation.renderedContent === 'string') {
      Object.defineProperty(materializedOperation, 'renderedContent', {
        value: operation.renderedContent,
        enumerable: false,
        configurable: true,
      });
    }
...
function dedupeOperationsByDestination(operations) {
  ...
  return operations.filter((operation, index) => (
    lastIndexByDestination.get(operation.destinationPath) === index
  ));
}
...
function createManifestInstallPlan(options = {}) {
  ...
  const operations = ['codex', 'cursor-cli', 'copilot'].includes(target)
    ? dedupeOperationsByDestination(materializedOperations)
