#include "tof_slam/ros1/imu_preprocessor.hpp"
...
void ImuPreprocessor::feed(const sensor_msgs::Imu::ConstPtr& msg) {
  std::lock_guard<std::mutex> lock(mutex_);

  ImuSample sample;
  sample.stamp = msg->header.stamp.toSec();
...
std::vector<ImuSample> ImuPreprocessor::consumeSamples(
    const ros::Time& t0, const ros::Time& t1) {
  std::lock_guard<std::mutex> lock(mutex_);
...
ros::Time ImuPreprocessor::getLatestTimestamp() const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (buffer_.empty()) return ros::Time(0);
  return ros::Time().fromSec(buffer_.back().stamp);
}
