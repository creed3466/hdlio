#include "tof_slam/ros2/imu_preprocessor.hpp"
...
void ImuPreprocessor::feed(const sensor_msgs::msg::Imu::SharedPtr& msg) {
  ...
  ImuSample sample;
  sample.stamp = msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9;
...
rclcpp::Time ImuPreprocessor::getLatestTimestamp() const {
  ...
  return rclcpp::Time(static_cast<int64_t>(buffer_.back().stamp * 1e9));
}
