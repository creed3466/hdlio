...
public:
  ...
  void feed(const sensor_msgs::msg::Imu::SharedPtr& msg);
  ...
  std::vector<ImuSample> consumeSamples(const rclcpp::Time& t0, const rclcpp::Time& t1);

  /// Get timestamp of latest buffered sample
  rclcpp::Time getLatestTimestamp() const;
